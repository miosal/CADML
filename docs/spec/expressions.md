...
Exponentiation (`^`) is not part of the 0.3 grammar; the parser
...
arguments are positional in 0.3 — the call site has no `name=value`
...
| `pow(base, exp)` | 2 | number | exponentiation — the canonical form (the `^` operator is not part of the 0.3 grammar; see §5) |
